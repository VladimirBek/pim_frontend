<template>
  <v-container fluid>
    <v-alert
      :model-value="!store.selectedPartner.blogger_offer_id"
      icon="$info"
      closable
      type="warning"
      text="Для создания приложения для данного контрагента необходимо завести заявку через телеграм бота"
    ></v-alert>
    ...
    <v-card class="information-container">
      ...
      <div class="information-section">
        ...
        <div class="information-section__container">
          ...
          <div v-if="store.selectedPartner.ogrnip || store.selectedPartner.blogger_offer?.legal_form==='ИП'" class="information-field">
    ...
    <v-card class="information-container">
      ...
      <div class="information-section">
        <div>
          ...
          <v-btn :variant="'flat'" :disabled="!store.selectedPartner.blogger_offer_id" color="primary" class="document-section__button" @click="openDocDialog('application')">Приложение</v-btn>
          <v-btn :variant="'flat'" :disabled="!store.selectedPartner.blogger_offer_id" color="primary" class="document-section__button" @click="openDocDialog('act')">Акт</v-btn>
        </div>
      </div>
      <div class="information-section">
        <div v-if="store.receivedFiles && store.receivedFiles.length > 0">
          ...
          <div class="partner-file__container">
            <div v-for="file in store.receivedFiles" :key="file.id" class="me-2 partner-file__item">
              ...
              <v-img :src="'data:image/jpeg;base64,' + file.code" @click="openFile('data:image/jpeg;base64,' + file.code, file)"/>
            </div>
            <div v-else>
              <v-btn icon="mdi-magnify" size="small" style="position: absolute" @click="openFile('data:application/pdf;base64,'+ file.code, file)"></v-btn>
    ...
    <DocumentDialog
    :is-shown="isDocDialog"
    :type="docDialogType"
    :role="role"
    @close-dialog="closeDocDialog"
    />
    <v-dialog :model-value="filePreview" max-width="1200px" persistent transition="dialog-top-transition">
        ...
        <v-card class="partner-dialog__container">
          ...
          <div class="partner-dialog__actions">
            <v-select v-model="store.openedFile.document_type" :items="docTypes" label="Тип документа" variant="outlined" width=200 hide-details class="partner-dialog__select"></v-select>
...
const openDocDialog = (type: string) => {
  store.openDocDialog();
